...
> ⚠️ **独立模式下的状态管理**：进入独立模式时，**必须**先执行状态管理初始化：
> ```bash
> # 初始化状态文件，前序步骤自动标记为 skipped
> ./scripts/state-manager.sh init <issue_id> "信号描述" S4_DIAGNOSIS
> # 进入 S4 步骤
> ./scripts/state-manager.sh step <issue_id>
> ```
> 独立模式下的前置步骤 1/2 仅在上下文中执行排查逻辑，**不单独走 S1/S2 状态流转**，但它们的输出结果必须在 S4 完成后通过 `done` 命令写入 JSON。
> 如果独立模式下前置步骤发现需要止损（🔴），需要额外补全 S2 和 S3 的状态记录。
...
**独立模式下**：输出完成后，询问用户是否需要继续执行代码修复（第五步）或生成处置报告（第六步）。若用户确认继续，需先补全 S1-S3 的状态记录（标记为 skipped 或 completed），再推进到对应步骤。
