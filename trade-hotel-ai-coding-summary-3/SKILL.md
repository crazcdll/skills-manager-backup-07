...
description: "同步酒店 AI-Coding 需求开发统计文档与本周需求列表，完成后自动发送大象消息提醒群成员更新 AI-Coding 数据。当用户提到「同步 AI-Coding」「更新 AI-Coding 需求列表」「整理 AI-Coding」「AI-Coding 统计更新」「酒店需求同步」「更新酒店 AI-Coding 文档」时激活。输入排期文档 + AI-Coding 文档两个学城链接，自动从排期文档提取本周需求池需求，生成需求列表子文档挂到 AI-Coding 文档下，再同步 AI-Coding 文档的酒店需求增删与链接修正，优先使用 PRD km 链接，最后发送大象消息通知有需求的研发同学更新 AI 成熟度与开发状态。"
...
从排期文档自动提取本周需求池需求，生成需求列表挂到 AI-Coding 文档下，再与上周 AI-Coding 统计文档对比同步酒店需求的增减与链接修正，最后发送大象消息通知群内有需求的研发同学更新 AI 成熟度与开发状态。
...
7. **新增**：需求列表中存在但 AI-Coding 中不存在的行。新增行从当前酒店续行克隆 `<td>` 结构、列宽和样式，只填充需求与研发同学列，其余统计字段保持空白；因此模板新增、删除或重排序列时无需改写固定列数。研发同学列生成蓝色 `<km-mention>`，empId 优先用排期文档携带的，其次从现有 XML 的 mention 反查（见「@ 人渲染机制」）
8. 更新 rowspan 数量 = 更新后的酒店行总数；方向单元格直接复用当前文档的酒店首行样式与人员信息
9. **表格结构预检**：输出前展开 `rowspan` / `colspan` 校验主统计表所有行的逻辑列数一致；不一致时同步脚本直接失败且不写出 XML，禁止上传
10. **脚注修复**：自动检测并移除 `<km-footnote-item>` 内的 `<del>` 标签
...
仅当步骤 5 日志中明确出现 `表格结构预检通过：所有行逻辑列数一致` 时，才允许上传。
...
若提示"当前有其他编辑者正在进行编辑操作"，重新执行步骤 4 获取最新 stepVersion，再重跑步骤 5~8。

### 8. 发送大象消息通知

使用 `catdesk daxiang send`（通过内置浏览器以用户身份发送，非机器人）通知群成员更新 AI-Coding 数据。

```bash
# 获取 AI-Coding 文档标题（用于消息中的链接标题）
DOC_TITLE="2026-08-20-AICoding需求开发统计"
DOC_URL="https://km.sankuai.com/collabpage/<ai_coding_id>"
...
脚本行为：
1. 从 `/tmp/requirement.md` 提取本周需求的所有研发同学（去重，保留首次出现顺序）
2. 在**发送前**逐人通过 `catdesk daxiang search` 将 mis 解析为大象数字 uid；每人默认重试 3 次
3. 只要任一人员未解析出与 MIS 完全匹配的数字 uid，整个通知直接失败，**不发送任何群消息**，禁止降级为纯文本 `@姓名`
4. 全部 uid 预检通过后，构造带原生蓝色 @mention 的单条消息并发送到群 `69695119485`（酒店民宿平台交易方向）
5. 可先用 `--dry-run` 做不发消息的预检，确认最终 URL、文案和全部 @mention 格式

消息格式（单条消息）：
```
第一行：https://km.sankuai.com/collabpage/2781649788   ← 链接卡片（--link-title）

第二行：更新下 ai coding 情况，AI成熟度分析、开发状态 一定要写，[@颜怡|mtdaxiang://...&isAt=true]
```

- 文档链接使用 `--link-title` 渲染为带标题的链接卡片（非 markdown 语法）
- `@人` 使用大象 `[@姓名|mtdaxiang://www.meituan.com/profile?uid=<uid>&isAt=true]` 语法，确保渲染为蓝色可点击链接
- uid 预检失败时中止发送并返回错误；不再降级为纯文本 `@姓名`

> 说明：群名称可能变更，但群 ID `69695119485` 不变。不使用 `--mentions` 参数，避免大象将 @人 前置到消息开头。

#### 消息测试安全规则

- **禁止**向正式业务群 `69695119485` 发送任何测试消息。
- 日常开发验证必须使用 `--dry-run`，它会完成 MIS→UID、消息 URL、原生 @mention 格式的完整预检，但绝不调用发送命令。
- 如需验证真实客户端渲染效果（链接卡片、蓝色 @mention），先向用户索取专用测试群 ID；只有用户明确提供后才允许向该测试群发送。
- 正式同步完成后、用户确认上传后发送到 `69695119485` 的通知属于业务动作，不是测试；仍需先完成全部 uid 与格式预检。

### 9. 清理临时文件

```bash
rm -f /tmp/aicoding.xml /tmp/aicoding_synced.xml /tmp/requirement.md /tmp/changes.json /tmp/requirement_subdoc.xml
```
...
AI-Coding 文档模板的统计字段可能调整（新增、删除或改名），因此**不再假定固定列数或固定列顺序**。同步脚本遵循以下约束：

- 以当前酒店首行去除“方向 rowspan 单元格”后的续行作为新增行模板；新增行仅替换需求与研发同学的内容，其他单元格清空并保留原列宽/样式。
- 首个输出酒店行通过复用当前方向单元格并更新 `rowspan` 生成，后续行保持续行模板结构。
- 输出前计算表头及每条数据行展开 `rowspan` / `colspan` 后的逻辑列数；任何一行不一致则阻止写出 XML 和上传。
- 标题匹配前对 XML 实体（如 `&amp;`）反转义，避免同一需求仅因编码差异被误判为删除和新增。
