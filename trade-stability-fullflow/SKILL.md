...
| 生成处置报告 | 复盘报告 | 输出问题总结、完整处置时间线并原样汇总前五步结果，写入可复用处置记忆 | [trade-stability-issue-report](references/trade-stability-issue-report/SKILL.md) |
...
**读取 [trade-stability-issue-report skill](references/trade-stability-issue-report/SKILL.md) 生成完整处置报告，并将可复用处置结论写入记忆**。
