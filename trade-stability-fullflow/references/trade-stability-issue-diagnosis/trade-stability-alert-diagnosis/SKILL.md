...
- **git 命令** — 克隆仓库、分析代码变更 diff
- **duo CLI（`@meishi/duo-cli`）** — DUO 页面查组件 git 地址
...
**diff 分析时额外关注**（结合线路 A 堆栈信息定向分析）：
- 与堆栈中出错**文件名/函数名**直接匹配的变更（优先）
- **组件注册**变更（新增/删除/修改组件）
- **状态管理**变更（store 结构变化、数据流变化）
- **第三方依赖**版本升级

**B 线结论判断**：
- **找到与告警强相关的变更** → 记录变更内容（文件路径 + 变更摘要），线路 B 得出结论
...
- **代码逻辑看起来正确，无明显问题** → 线路 C 无结论，进入第三步汇总分析（扩大排查）
