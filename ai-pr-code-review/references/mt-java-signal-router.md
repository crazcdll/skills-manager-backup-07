...
| SIG-THRIFT | `api-compat/ha-10-api-compat.md`, `api-compat/java-08-api-design.md` | P0: MT:HA-API001(字段删除/改类型，**仅限跨部署单元契约 + 需调用方反查证据**), MT:HA-API004(Thrift IDL兼容); P1: MT:HA-API002/003 | 无重叠（CR: 无 Thrift/API 兼容性规则）。⚠️ MT:HA-API001 判 P0 前必须校验前置条件：载体为 Thrift IDL / 对外 interface / 二方库 public API / 序列化 DTO，且附带外部调用点「文件+行号」。`*Impl` 内部签名变更 / 无调用方证据 → 最高 P2 |
| SIG-CONCUR | `concurrency/java-04-concurrency.md`, `concurrency/ha-07-threadpool.md` | P0: MT:HA-J001(Executors禁用/无界队列), MT:HA-J005(Future超时); P1: MT:HA-J002/J003; P2: JAVA-04 | MT:HA-J001 与 CR:RM-11/CR:RM-12 重叠 → Executors 禁用 + 无界队列以 CR: 为主；MT: 只报 CR: 未覆盖的（DiscardPolicy 禁用、局部线程池 shutdown） |
| SIG-RPC | `rpc/ha-01-failure-design.md`, `rpc/ha-02-observability.md` | P0: MT:HA-O004~O007(日志强制); P1: MT:HA-F001~F006(失败设计), MT:HA-O001(Metrics) | MT:HA-F001 与 CR:FT-01(RPC超时) 部分重叠 → 超时以 CR:FT-01 为主；MT: 只报 CR: 未覆盖的（failover 策略、降级链路、重试退避） |
...
| SIG-EXCEPTION | `exception/java-05-exception-full.md`, `exception/ha-03-defensive.md` | P0: MT:HA-D005~D011(强制); P1: MT:JAVA-05(异常处理完整), MT:HA-D003/D004(异常分类/资源释放) | MT:JAVA-05(N001/N003~N009) 与 CR:EH-01/02/04/05 部分重叠 → 空 catch/catch 过宽以 CR: 为主；MT: 只报 CR: 未覆盖的（异常分类、业务异常 vs 系统异常分离、Throwable 捕获场景） |
...
| SIG-CONFIG | `config/ha-11-config-safety.md` | P0: MT:HA-CF004(banma_util禁用); P1: MT:HA-CF001~CF003 | 无重叠（CR: 无配置管理规范） |
