...
def cmd_pr_comments(args, cookies):
    ...
    if not assign_data.get("error"):
        for item in assign_data.get("values", []):
            for cw in item.get("comments", []):
                comment = cw.get("comment", {})
                anchor = cw.get("commentAnchor", {})
                assignment = comment.get("assignment", {})
                comments.append({
                    "id": comment.get("id"), "type": "assignment",
                    "state": assignment.get("state"),
                    "author": comment.get("author", {}).get("name"),
                    "text": comment.get("text"),
                    "file": anchor.get("path") or item.get("path"),
