...
适用场景：**DUO 转 H5 / MAX 转 H5 / i 版 H5**（在浏览器或 WebView 中运行的 H5 页面）。

H5 有前端异常上报（Raptor H5 项目），同时支持后端日志查询。H5 无 UUID，前端异常按 projectId + 时间范围查询，后端日志按 userId / 手机号 / traceId 等标识查询。
...
**线路A（日志查询）入口选择**：
- 提供了 **userId 或手机号** → A0 查 H5 前端异常（并行）+ A1 查后端日志
...
| `result.table.rows[].data5` | 容器类型（H5 / WebView） |
...
> i 版专用链接：`aHR0cHM6Ly9yYXB0b3IubXdzLnNhbmt1YWkuY29tL2Zyb250ZW5kL2Vycm9yL2xpc3Q/cHJvamVjdElkPTI3MTRg
...
| `traceId` / `props.headers["M-TRACEID"]` | 请求 traceId | 查后端日志 |
| `userId` / `commonParams.userInfo.userId` | 用户 userId | 查后端日志的查询条件 |
...
**⚠️ i 版特殊处理**：
- i 版 403 问题参考：`aHR0cHM6Ly9rbS5zYW5rdWFpLmNvbS9wYWdlLzEzNTA2NjY3NDJg
- i 版日志查询：`aHR0cHM6Ly9yYXB0b3IubXdzLnNhbmt1YWkuY29tL2xvZy90b3BpYy92aWV3L25lc3RfY29tLnNhbmt1YWkubWVpc2hpLmZlLmlfcHJvZA==`
- i 版 nest 服务：`aHR0cHM6Ly9uZXN0LnNhbmt1YWkuY29tL3ByZXZpZXcjL3NlcnZpY2UvY29tLnNhbmt1YWkubWVpc2hpLmZlLmkvZnVuY3Rpb24vMTI0MDUvbG9nYA==
...
**后端无日志** → 按场景兜底：
  - 网关层问题 → 查网关日志：
    ```
    aHR0cHM6Ly9vY2VhbnVzLm13cy5zYW5rdWFpLmNvbS9zaXRlX2RldGFpbD9zaXRlX25hbWU9cnBjLm1laXR1YW4uY29tJnNpdGVfaWQ9MTAxNTcmdGFiPWxvZy1zZWxlY3Q=
    ```
  - i 版问题 → 查 i 版日志：
    ```
    aHR0cHM6Ly9yYXB0b3IubXdzLnNhbmt1YWkuY29tL2xvZy90b3BpYy92aWV3L25lc3RfY29tLnNhbmt1YWkubWVpc2hpLmZlLmlfcHJvZA==
    ```
...
完整步骤参见公共文档：[code-analysis.md](../../references/code-analysis.md#线路-cmaster-最新代码分析无变更但问题与代码相关时执行)

**C 线结论判断**：
- **找到明确的代码问题** → 记录文件路径 + 代码行 + 问题说明，线路 C 得出结论
- **发现受配置/实验控制** → 记录 Horn key 或实验名，建议检查线上配置状态，线路 C 得出结论
...
| 无变更，线路 A 无结论，问题与代码相关 | 启动线路 C（clone master 分析），以线路 C 结论为根因；线路 C 无结论则参考 useful-links.md |
| 无变更，线路 A 无结论，问题与代码无关 | 无法定位，参考 useful-links.md 推荐辅助工具扩大排查 |
| 线路 C 有结论 | 以 master 代码逻辑缺陷/配置控制为根因，进入输出 |
