#!/usr/bin/env python3
"""
子命令：hotel-testdata fullday

全日房端到端上单（路径 A/B/C/D 自动 infra → W1 batchCreateGoods → 上线 → 缓存刷新）

流程：
  1. 根据用户提供的 ID 推断路径，执行 infra 构造（POI/供应商/合同/房型）
  2. 使用 infra 产出的 partnerId/poiId/roomId/roomName/contractNo 创建全日房
  3. 自动上线 + 库存补偿（含"90天库存"失败重试） + 缓存刷新

路径推断规则（与 skill w8-infra-bootstrap.md 完全一致）：
  partnerId + poiId 均有 → 路径B（私海认领+绑定门店+查/建合同+建房型）
  仅有 partnerId         → 路径D（建POI+认领+绑定+查/建合同+建房型）
  仅有 poiId             → 路径C（数据池查供应商+认领+绑定+查/建合同+建房型）
  什么都没有             → 路径A（建POI+认领+建供应商+等待+查/建合同+建房型）

⚠️ 注意：
  - 全日房 contractNo 通过 --set goodsDetailList.0.goodsBaseInfo.contractNo=<值> 传入
    （不是 --contract-no 参数）
  - 路径A 新建供应商时供应商约需 1 分钟就绪，程序自动等待
  - 境外场景要求门店/供应商/房型全部是境外的
...
用法示例：
  ...
  # 已有 partnerId + poiId，并自定义取消政策（免费取消）
  hotel-testdata fullday --partner-id 4550589 --poi-id 1085927256096396 \\
  ...
  # 已有基础实体，直接上单（跳过 infra，需同时提供 --room-id --room-name --contract-no）
  hotel-testdata fullday --partner-id 4550589 --poi-id 1085927256096396 \\
  ...
  # dry-run（不执行，只打印参数）
  hotel-testdata fullday --partner-id 4550589 --poi-id 1085927256096396 --dry-run
"""
...
def _get_mod():
    """懒加载 factory/fullday/create-fullday.py 模块。"""
    return get_factory("fullday")
...
def create_fullday(
    ...
    """全日房全链路创建（直接调 factory 层，无 goods 中间层）。"""
    mod = _get_mod()
    ...
    # 2b. 自动命名
    _user_set_name = overrides and any(
    ...
    if not _user_set_name and (not goods_name or _is_overseas_multi):
        auto_goods_name, auto_rp_name, auto_rp_custom_name = mod._build_names(room_name, params)
        ...
        print(f"[自动命名] goodsName={auto_goods_name}")
...
def add_subparser(subparsers: argparse._SubParsersAction) -> None:
    ...
    p = subparsers.add_parser(
        "fullday",
        help="全日房端到端上单（路径A/B/C/D infra → batchCreateGoods → 上线）",
    ...
    p.add_argument("--contract-no", default="", help="已有合同号（字符串，如 ZSFW-A9-75178816）")

    # ── 门店/供应商参数（路径A/C/D 新建时）────────────────────────────────
    p.add_argument("--city",         default="北京", help="创建门店城市（路径A/D，默认北京）")
    p.add_argument("--overseas",     action="store_true", help="境外场景")
    p.add_argument("--partner-type", type=int, default=2,
                   help="供应商类型：2=境内自采预付，3=境外，9=女娲（默认2）")
    ...
    p.add_argument(
        ...
        help="覆盖任意字段，可多次使用。KEY 为点分路径。\n"
             "例：--set goodsDetailList.0.rpInfo.rpCancelModel.normalRule.cancelItemType=1\n"
             "    --set goodsDetailList.0.goodsBaseInfo.contractNo=ZSFW-A9-75178816\n"
             "⚠️  全日房 contractNo 必须通过 --set 传入，或使用 --contract-no 参数",
    )
...
def _run(args: argparse.Namespace) -> None:
    ...
    # ── 如果 --contract-no 传了，注入到 overrides（等同 --set contractNo）──
    if args.contract_no:
    ...
    print("\n" + "═" * 60)
    print("🚀 hotel-testdata fullday（全日房端到端上单）")
    print("═" * 60)
    if args.dry_run:
    ...
    if has_all_ids:
        ...
        print(f"     contractNo : {contract_no or '（未传，通过 --set 传入）'}")
    else:
        ...
        print("\n" + "─" * 60)
        print("  ✅ 基础实体就绪，开始创建全日房")
        print(f"     partnerId  : {partner_id}")
        print(f"     poiId      : {poi_id}")
        print(f"     roomId     : {room_id}")
        print(f"     roomName   : {room_name}")
        print(f"     contractNo : {contract_no or '（无合同，接口可能拦截）'}")
        print("─" * 60)

    # ── 如果 contractNo 已知但 overrides 里没有，注入到 overrides ──────────
    if contract_no and "goodsDetailList.0.goodsBaseInfo.contractNo" not in overrides:
        overrides["goodsDetailList.0.goodsBaseInfo.contractNo"] = contract_no

    # ── 创建全日房 ──────────────────────────────────────────────────────────
    goods_id = create_fullday(
        partner_id=str(partner_id),
        poi_id=str(poi_id),
        room_id=int(room_id),
        room_name=room_name,
        goods_name=args.goods_name,
        contract_no=None,  # 已通过 overrides 注入，这里不重复传（避免双写）
        overrides=overrides,
        swimlane=args.swimlane,
        dry_run=args.dry_run,
