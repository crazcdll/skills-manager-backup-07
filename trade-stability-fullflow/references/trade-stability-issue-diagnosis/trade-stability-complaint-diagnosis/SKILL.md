...
- **线路 B**：仅有 Diva 变更时执行；统一读取公共文档 [code-analysis.md](../../../references/code-analysis.md#线路-b变更代码分析有-diva-变更时执行)。
- **线路 C**：无 Diva 变更且问题判断与代码相关时执行；同样统一读取 [code-analysis.md](../../../references/code-analysis.md#线路-c默认分支代码分析无变更但问题与代码相关时执行)。
...
| 变更内容 | {全量 diff 中与问题相关的文件、代码位置、变更摘要和关联证据；无关联则明确说明} |
